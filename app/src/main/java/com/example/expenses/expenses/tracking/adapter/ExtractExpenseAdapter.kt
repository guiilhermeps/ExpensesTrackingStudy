...
class ExtractExpenseHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
    ...
    private fun ImageView.setIconForExpense(isIncomingValue: Boolean) {
        if (isIncomingValue) {
            setImageDrawable(
                    itemView.resources.getDrawable(R.drawable.ic_seta_baixo, itemView.context.theme)
            )
        } else {
            setImageDrawable(
                    itemView.resources.getDrawable(R.drawable.ic_seta_cima, itemView.context.theme)
            )
        }
    }
